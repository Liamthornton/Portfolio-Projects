...
--- Vehicle Type ---
    ,  Sum(CASE WHEN o.vehicle_name__c != 'Light Commercial Vehicle' OR o.vehicle_name__c IS NULL
                                                                        THEN 1 END)     AS Car
    ,  Sum(CASE WHEN o.vehicle_name__c = 'Light Commercial Vehicle'     THEN 1 END)     AS LCV
--- Car mileage ---
...
--- Fuel Type ---
    ,   SUM(CASE WHEN o.fueltype__c in ('PETROL','Petrol') THEN 1 END)                  AS Petrol
    ,   SUM(CASE WHEN o.fueltype__c in ('DIESEL','Diesel') THEN 1 END)                  AS Diesel
    ,   SUM(CASE WHEN o.fueltype__c in ('Petrol/Electric','Diesel/Electric','HYB-PETROL','PETROL/ELE','HYB-DIESEL') THEN 1 END)
                                                                                        AS Hybrid
    ,   SUM(CASE WHEN o.fueltype__c in ('ELECTRIC', 'Electric (Battery)') THEN 1 END)   AS Electric
    ,   SUM(CASE WHEN o.fueltype__c in ('PETROL/GAS','GAS PETROL') THEN 1 END)          AS LPG
    ,   SUM(CASE WHEN o.fueltype__c not in ('PETROL','Petrol','DIESEL','Diesel', 'UNKNOWN',
                                           ...
                                           'ELECTRIC', 'Electric (Battery)','PETROL/GAS','GAS PETROL') AND o.fueltype__c IS NOT NULL THEN 1 END)
                                                                                        AS FuelOther
    ,   SUM(CASE WHEN o.fueltype__c IS NULL or o.fueltype__c='UNKNOWN' THEN 1 END)      AS FuelUnknown
...
--- Vehicle Type ---
    ,  Sum(CASE WHEN o.vehicle_name__c != 'Light Commercial Vehicle' OR o.vehicle_name__c IS NULL THEN la.total_finance_amount END)
                                                                                                              AS Car
    ,  Sum(CASE WHEN o.vehicle_name__c = 'Light Commercial Vehicle' THEN la.total_finance_amount END)         AS LCV
--- Car mileage ---
    ,   Floor(100*AVG(la.mileage))/100                                                  AS AVGMileage
...
--- Fuel Type ---
    ,   SUM(CASE WHEN o.fueltype__c in ('PETROL','Petrol') THEN la.total_finance_amount END)                  AS Petrol
    ,   SUM(CASE WHEN o.fueltype__c in ('DIESEL','Diesel') THEN la.total_finance_amount END)                  AS Diesel
    ,   SUM(CASE WHEN o.fueltype__c in ('Petrol/Electric','Diesel/Electric','HYB-PETROL','PETROL/ELE','HYB-DIESEL') THEN la.total_finance_amount END)
                                                                                        AS Hybrid
    ,   SUM(CASE WHEN o.fueltype__c in ('ELECTRIC', 'Electric (Battery)') THEN la.total_finance_amount END)   AS Electric
    ,   SUM(CASE WHEN o.fueltype__c in ('PETROL/GAS','GAS PETROL') THEN la.total_finance_amount END)          AS LPG
    ,   SUM(CASE WHEN o.fueltype__c not in ('PETROL','Petrol','DIESEL','Diesel', 'UNKNOWN',
                                           'Petrol/Electric','Diesel/Electric','HYB-PETROL','PETROL/ELE','HYB-DIESEL',
                                           'ELECTRIC', 'Electric (Battery)','PETROL/GAS','GAS PETROL') AND o.fueltype__c IS NOT NULL THEN la.total_finance_amount END)
                                                                                        AS FuelOther
    ,   SUM(CASE WHEN o.fueltype__c IS NULL or o.fueltype__c='UNKNOWN' THEN la.total_finance_amount END)      AS FuelUnknown
FROM Loan_Aggs AS la
LEFT JOIN salesforce_ownbackup_ext.opportunity_calculated oc on la.opportunity_id=oc.id
LEFT JOIN oodledata.dealer_dim dd on la.introducer_id=dd.dealer_id
LEFT JOIN salesforce_ownbackup_ext.opportunity o on la.opportunity_id=o.id
